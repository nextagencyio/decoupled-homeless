...
function PageNotFound({ path }: { path: string }) {
  return (
    <div className="text-center py-16">
      <div className="bg-white border-l-4 border-primary-700 shadow-sm p-12">
    ...
    if (!entity) {
      return (
        <div className="min-h-screen bg-[#faf8f5]">
          <Header />
          <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12"><PageNotFound path={path} /></main>
        </div>
      )
    }
    ...
    return (
      <div className="min-h-screen bg-[#faf8f5]">
        ...
        <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
          <ErrorBoundary>
            <article className="bg-white border-l-4 border-primary-700 shadow-sm overflow-hidden">
              {image && (<ResponsiveImage image={image} alt={image.alt || title} className="rounded-t-lg" priority={true} />)}
              <div className="p-6 md:p-8">
                <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-serif font-medium text-gray-900 mb-4 md:mb-6">{title}</h1>
                <div className="w-16 h-0.5 bg-accent-600 mb-6" />
