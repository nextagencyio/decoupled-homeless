...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!item) {
    return { title: 'Resource Not Found | Homeless' }
  }
              ...
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).provider && (
                    <div>
                      <dt className="text-sm text-gray-500">Provider Organization</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).provider}</dd>
                    </div>
                  )}
                  {(item as any).phone && (
                    <div>
