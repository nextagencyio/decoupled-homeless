...
import { GET_SHELTER_BY_PATH } from '@/lib/queries'
import { DrupalShelter } from '@/lib/types'
...
interface ShelterByPathData {
  route: {
    entity: DrupalShelter
  } | null
}

async function getShelter(path: string): Promise<DrupalShelter | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<ShelterByPathData>({
      query: GET_SHELTER_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/shelters/${slug.join('/')}`
  const item = await getShelter(path)

  if (!item) {
    return { title: 'Shelter Not Found | Homeless' }
  }
                ...
                <dl className="space-y-4">
                  {(item as any).capacity && (
                    <div>
                      ...
                      <dd className="font-semibold text-gray-900">{(item as any).capacity}</dd>
                    </div>
                  )}
                  {(item as any).address && (
                    <div>
                  ...
                  {(item as any).hours && (
                    <div>
                      <dt className="text-sm text-gray-500">Operating Hours</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).hours}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
