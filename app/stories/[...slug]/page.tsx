...
import { GET_IMPACT_STORY_BY_PATH } from '@/lib/queries'
...
interface ImpactStoryByPathData {
  route: {
    entity: DrupalImpactStory
  } | null
}
...
async function getImpactStory(path: string): Promise<DrupalImpactStory | null> {
  try {
    ...
    const { data } = await apolloClient.query<ImpactStoryByPathData>({
      query: GET_IMPACT_STORY_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/stories/${slug.join('/')}`
  const item = await getImpactStory(path)

  if (!item) {
    return { title: 'Impact Story Not Found | Homeless' }
  }

  return {
    title: `${item.title} | Homeless`,
              ...
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).personName && (
                    <div>
                      <dt className="text-sm text-gray-500">Person Name</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).personName}</dd>
                    </div>
                  )}
                  {(item as any).programParticipated && (
                    <div>
                      <dt className="text-sm text-gray-500">Program Participated</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).programParticipated}</dd>
                    </div>
                  )}
                  {(item as any).outcome && (
                    <div>
                      <dt className="text-sm text-gray-500">Outcome</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).outcome}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
